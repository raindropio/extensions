	...
	render() {
		...
		if (this.state.show)
			items = this.items.map((item,index)=>{
				if (item){
					item._id = this.props.fromId-1-index;
					item.cover = this.getCover(item.cover_path);

					return <Item key={"item"+index}
								active={(this.props.selected == item._id)}
								item={item}
		...
		return (
			<section>
				<div className="group" onClick={this.toggleShow.bind(this)}>
					<div className="title">{t.s("suggested")+" "+t.s("collectionsCount")}</div>
